...
The owner manages **products** and **customer reviews** from a private admin page at `https://jolkona.dpdns.org/admin.html` using Google sign-in + Firestore Security Rules — only allowlisted admin emails (`jolkona2007@gmail.com`) can write; visitors get read-only access. Content is stored in **Cloud Firestore** and appears on the live shop instantly, with the built-in `js/products.js` data kept as a fallback. See **[ADMIN-SETUP.md](ADMIN-SETUP.md)** for the one-time Firebase setup.
